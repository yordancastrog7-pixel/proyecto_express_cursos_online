...
export async function findAll(skip: number, limit: number) {
  const [data, total] = await Promise.all([
    Course.find().populate('category').sort({ createdAt: 1 }).skip(skip).limit(limit).lean(),
...
export async function update(id: string, dto: UpdateCourseDto) {
  try {
    const updated = await Course.findByIdAndUpdate(id, dto, { returnDocument: 'after', runValidators: true })
      .populate('category')
      .lean();
    if (!updated) {
      throw new AppError(404, 'Curso no encontrado');
