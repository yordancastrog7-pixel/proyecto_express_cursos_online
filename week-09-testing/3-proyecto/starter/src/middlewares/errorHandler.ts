...
export function errorHandler(err: Error, req: Request, res: Response, _next: NextFunction): void {
  ...
  // JSON mal escrito en el body (ej. una coma de más): es culpa del cliente (400), no un bug del servidor.
  if ((err as Error & { type?: string }).type === 'entity.parse.failed') {
    const response: ErrorResponse = { error: 'Bad Request', message: 'El cuerpo de la petición no es un JSON válido' };
