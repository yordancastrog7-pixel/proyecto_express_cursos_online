...
import { ROLES, Role, isRole } from '../config/roles';
...
function toPublicUser(user: {
  ...
  return {
    ...
    name: user.name,
    email: user.email,
    role: isRole(user.role) ? user.role : ROLES.USER,
...
// El rol se lee de la base de datos cada vez que se emiten tokens, así que un
// cambio de rol se refleja en el siguiente login o refresh.
async function issueTokens(userId: string, role: Role): Promise<TokenPair> {
  const accessToken = tokens.signAccessToken(userId, role);
...
export async function login(dto: LoginDto): Promise<{ user: PublicUser; tokens: TokenPair }> {
  ...
  return { user: publicUser, tokens: await issueTokens(publicUser.id, publicUser.role) };
}

export async function listUsers(): Promise<PublicUser[]> {
  return (await usersRepo.findAll()).map(toPublicUser);
}
...
export async function refresh(refreshToken: string | undefined): Promise<TokenPair> {
  ...
  return issueTokens(userId, toPublicUser(user).role);
}
